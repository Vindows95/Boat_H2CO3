...
public class MioCustomButton extends com.google.android.material.button.MaterialButton
{
	...
	public void 初始化(){
		...
		setAllCaps(false);
		setX(MioCustomUtils.percentTopx(位置.get(0), H2CO3CustomManager.scrennWidth));
		setY(MioCustomUtils.percentTopx(位置.get(1), H2CO3CustomManager.screenHeight));
	}
	
	
	public void 刷新(){
		List<Double> 位置=new ArrayList<Double>();
		位置.add(MioCustomUtils.pxToPercent(this.getX(), H2CO3CustomManager.scrennWidth));
		位置.add(MioCustomUtils.pxToPercent(this.getY(), H2CO3CustomManager.screenHeight));
		this.位置.clear();
		this.位置.addAll(位置);
	}
	...
	public void set位置(List<Double> 位置) {
		this.位置 = 位置;
	}

	public List<Double> get位置() {
		return 位置;
	}
