...
public class H2CO3CustomManager implements OnTouchListener,CompoundButton.OnCheckedChangeListener,OnClickListener {
	...
	public interface 容器事件回调 {
		void 按下(MotionEvent 事件);
		void 移动(MotionEvent 事件);
		void 抬起(MotionEvent 事件);
		void 点击事件();
		void 长按事件();
	...
	public interface 自定义按键回调 {
		void 命令接收事件(String command);
		void 键值接收事件(short 键值, boolean 按下);
		void 控制鼠标指针移动事件(int x, int y);
		void 鼠标回调(short 键值, boolean 按下);
		void 按下();
		void 抬起();
	}
    public interface 圆形菜单回调 {

    }
	private Handler 容器事件回调长按计时器=new Handler();
	private Runnable 容器事件回调长按任务=new Runnable(){
		@Override
		public void run() {
			容器事件回调.长按事件();
		}
	...
	public void 初始化(Context context, ViewGroup 容器, String 储存路径) {
		if (!是否已初始化) {
			...
			this.容器 = 容器;
			设置屏幕高宽(容器.getHeight(), 容器.getWidth());
			键值转换器 = new Translation(KeyEvent.KEYMAP_TO_X);
			set储存路径(储存路径);
			反序列化();
			添加按键至容器();
			是否已初始化 = true;
			目标按键标识 = new ArrayList<String>();
		}


	}
	public void 设置容器事件回调(容器事件回调 容器事件回调) {
		this.容器事件回调 = 容器事件回调;
	}
	...
	private void 反序列化() {
		...
		builder.registerTypeAdapter(MioCustomButton.class, new H2CO3Deserializer(context));
		mGson = builder.setPrettyPrinting().create();
		自定义按键集合 = new ArrayMap<String,MioCustomButton>();
		try {
			FileReader reader=new FileReader(new File(储存路径, "H2CO3KeyBoard.json"));
			...
			for (int i=0;i < mJSONArray.length();i++) {
				...
				自定义按键集合.put(mMioCustomButton.getID(), mMioCustomButton);
	...
	public void 创建按键() {
		try {
			...
			json.put("CornerRadius", getText(alert_miocustom_main_edit_round));
			...
			json.put("默认隐藏", true);
			json.put("command", getText(alert_miocustom_main_edit_cmd));
    ...
    public void 保存按键() {
		try {
			FileOutputStream out=new FileOutputStream(new File(储存路径, "H2CO3KeyBoard.json"));
			int i=1;
...
//            容器.addView(触摸板,params);
	...
	public void onClick(View v) {
		if (v == alert_miocustom_main_tv_height_left) {
			...
			按键选择模式 = true;
			try {
				if (当前按键.get按键类型().equals("显隐控制按键") && 当前按键.is默认隐藏()) {
	...
	private long 按下时间;
	private Handler 长按计时器=new Handler();
	private Runnable 长按计时任务=new Runnable(){
		@Override
		public void run() {
		...
		if (自定义开关 && !按键选择模式) {
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				if ((System.currentTimeMillis() - 按下时间) < 100) {
					...
					alert_miocustom_main_edit_round.setText("" + 当前按键.getCornerRadius());
...
//					}
				...
				长按计时器.removeCallbacks(长按计时任务);
				FrameLayout.LayoutParams params=(FrameLayout.LayoutParams)当前按键.getLayoutParams();
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				if (目标按键标识.contains(选择按键.getID())) {
					目标按键标识.remove(选择按键.getID());
					选择按键.setTextColor(Color.parseColor(选择按键.get文本颜色()));
					Toast.makeText(context, "已取消选择按键:" + 选择按键.get文本(), Toast.LENGTH_SHORT).show();
				} else {
					目标按键标识.add(选择按键.getID());
					选择按键.setTextColor(Color.RED);
					Toast.makeText(context, "已选择按键:" + 选择按键.get文本(), Toast.LENGTH_SHORT).show();
				}
			}
		} else {
			...
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				if (当前按键.is控制鼠标指针()) {
					...
					自定义按键回调.控制鼠标指针移动事件((int)xx, (int)yy);
				}
			} else if (event.getAction() == MotionEvent.ACTION_UP) {
				if (当前按键.get按键类型().equals("普通按键")) {
					if (当前按键.is控制鼠标指针()){
	...
	public void set指针横坐标(int 指针横坐标) {
		this.指针横坐标 = 指针横坐标;
	}
	...
	public void set指针纵坐标(int 指针纵坐标) {
		this.指针纵坐标 = 指针纵坐标;
	}
...
class H2CO3Deserializer implements JsonDeserializer<MioCustomButton> {
	Context mContext;
	@Override
