...
//            容器.addView(触摸板,params);
	...
	public void onClick(View v) {
		if (v == alert_miocustom_main_tv_height_left) {
			int tmp=Integer.valueOf(getText(alert_miocustom_main_edit_height));
			...
			int tmp=Integer.valueOf(getText(alert_miocustom_main_edit_width));
			...
			int tmp=Integer.valueOf(getText(alert_miocustom_main_edit_textsize));
			...
			int tmp=Integer.valueOf(getText(alert_miocustom_main_edit_round));
			alert_miocustom_main_edit_textsize.setText(String.valueOf(tmp -= 1));
...
class MioDeserializer implements JsonDeserializer<MioCustomButton> {
	Context mContext;
	@Override
